...
function selectAutoriai($id) {
    ...
    if ($result = mysqli_query($this->mysqli, $query)) {
        ...
        $temp = new Naujienos($row['fk_Autorius']);
        $temp->id = $row['id'];
        return $temp;
    }
}
...
function selectManyAutoriai($object, $where = null) {
    global $mysqli;
    if ($where == null) {
        $where = "";
    } else {
    ...
    if ($result = mysqli_query($this->mysqli, $query)) {
        while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
            ...
            $temp = new zanrai($row['fk_Autorius']);
            $temp->id = $row['id'];
            $result[] = $temp;
        }
    }
...
function insertAutoriai($object) {
    ...
    $query = "INSERT INTO Autoriai (fk_Knyga, fk_Autorius) VALUE (
          ...
          '" . mysqli_real_escape_string($this->mysqli, $this->fk_Autorius) . "
...
function removeAutoriai($object) {
    global $mysqli;
    $query = "DELETE FROM Autoriai WHERE id = " . $this->id;
    mysqli_query($this->mysqli, $query);
}
